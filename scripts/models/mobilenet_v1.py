...
def model_stem(alpha=0.25, **kwargs) -> str:
    return f"mobilenet_v1_{int(alpha * 100):03d}"


def build_model(alpha=0.25, **kwargs) -> tf.keras.Model:
...
def get_representative_data(alpha=0.25, **kwargs):
    rng = np.random.default_rng(42)
    (x_train, _), _ = get_dataset()
    indices = rng.choice(len(x_train), size=1000, replace=False)
    for idx in indices:
